...
// 24.08 Pareto filter added
// 24.08 single drone 
// 26.08 basis functions replaced
*/
...
function [G,b,s] = circulation(n,rho,u,L,ar,cl_avg,A2,A4,A6,A8,A10,A12)
...
//    ds = b / (n+1); 
    ...
    G = G0 * sqrt( 1 - (2*s/b).^2 ) + A2 * h2(2*s/b) + A4 * h4(2*s/b) + A6 * h6(2*s/b) + A8 * h8(2*s/b) + A10 * h10(2*s/b) + A12 * h12(2*s/b);
...
// circulation around trailer drones wings 
    ...
    [G,b,s] = circulation(n,rho,u,L,ar,cl_avg,A2,A4,A6,A8,A10,A12);
...
ga_params = add_param(ga_params,'minbound',[2.5;  24.25; 31; 1.25; 15; -0.50*ones(6,1)]); //-0.5*ones(12,1)]);
ga_params = add_param(ga_params,'maxbound',[3.5;  25;    32; 1.35; 16; +0.50*ones(6,1)]); // +0.5*ones(12,1)]);
...
plot(F_out(:,1),A2TA,'bo'); 
//plot(fobj_pop_opt(:,1),A3TA,'go'); 
plot(F_out(:,1),A4TA,'r*'); 
//plot(fobj_pop_opt(:,1),A5TA,'b*'); 
plot(F_out(:,1),A6TA,'g*'); 
plot(F_out(:,1),A8TA,'bo'); 
plot(F_out(:,1),A10TA,'ro'); 
plot(F_out(:,1),A12TA,'go'); 
xlabel('$\LARGE \mu_1$');
...
for ii= 1:PF_size; 
    [G,b,s] = circulation(601,rho,u(ii),L,ar(ii),cl(ii),A2TA(ii),A4TA(ii),A6TA(ii),A8TA(ii),A10TA(ii),A12TA(ii)); 
    ww=inducedvelocity(601,b,G);
