...
function dfds = fdd(f,s)
    ...
    dfds = max(min(dfds,100),-100);

endfunction

function [G,b,s,ds] = circulation(n,rho,u,L,ar,cl_avg,cla,A1,A2,A3,A4,A5,A6)
...
//    L = 750.0; 
    ...
    [G,b,s,ds] = circulation(n,rho,u,L,ar,cl_avg,cla,A1,A2,A3,A4,A5,A6);
    ww    = inducedvelocity(n,b,G);
    [G_LA,b,s,ds] = circulation(n,rho,u,L_LA,ar,cl_avg,cla,A1_LA,A2_LA,A3_LA,A4_LA,A5_LA,A6_LA);
...
x_min = [0.50; -20.5*ones(12,1)];  
x_max=  [2.00; +20.5*ones(12,1)];  
...
// [fopt, xopt] = optim (cost, "b", x_min, x_max, x0, "gc", "ar", 3, 500)    
